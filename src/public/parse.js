/* AYP TrackID 抽出ロジック（純粋関数・DOM非依存）— ブラウザとNodeテスト両対応 */
(function (root, factory) {
  if (typeof module !== 'undefined' && module.exports) module.exports = factory();
  else root.ShotParse = factory();
  ...
  function plausibleText(l, minLetters) {
    const n = l.trim();
  ...
  function relScore(r, title, artist) {
    const tok = (s) => String(s).toLowerCase().split(/[^a-z0-9ぁ-んァ-ヶ一-龯]+/).filter((w) => w.length >= 2);
    const qT = tok(title);
    const qA = tok(artist);
    if (!qT.length && !qA.length) return 0;
    const rt = String(r.title || '').toLowerCase();
    const ra = String(r.artist || '').toLowerCase();
    const norm = (s) => String(s).toLowerCase().replace(/[^a-z0-9ぁ-んァ-ヶ一-龯]+/g, ' ').trim();
    let s = 0;
    if (qT.length) {
  ...
  return { parseOcrText, extractElapsed, extractEpisode, keepLine, relScore };
});
